...
<div id="wrapper">
	<div id="body">
	  ...
	  <div id="content">
		...
		ResultSet rs=st2.executeQuery("select * from registration where username='"+user+"'");
		while (rs.next())
		{
		count++;
		}
		...
		st.executeUpdate("insert into login values('" + user + "','" + pass + "','" + stat + "')");
                st1.executeUpdate("insert into registration(name,username,secretquestion,answer,address,pincode,phone,email,village,taluk,district,state) values('" + na + "','" + user + "','" + se + "','" + an + "', '" + add + "'," + pin + "," + phone + ",'" + email + "','" + vill + "','" + taluk + "','" + dis + "','" + state + "')");
                out.println("Congrats!!  Now you are a registered Member");
		}
		else out.println("User already registered !!");
            } catch (Exception e) {
                out.println(e);
            }
    ...
    %>
     <br />
		...
		<div id="footer">
			Copyright @ 2016, Appic. All rights Reserved.
		</div>
	</div>
</div>
</body>
